...
/*******w******** 
    ...
    Description: This file edits an existing articles.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($title) && ($content) && ($id)) {
        ...
        $query = "UPDATE articles SET title = :title, content = :content, created_at = NOW(), updated_at = NOW()  WHERE id = :id";
    ...
    if (isset($_GET['id'])) {
        ...
        if ($id) {
            $query = "SELECT id, title, content, updated_at FROM articles WHERE id = :id";
            $statement = $db->prepare($query);
